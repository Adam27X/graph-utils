...
#include "../../graph-utils/multi_search/transitive_closure/transitive_closure.cuh"
...
bool verify(host_graph &g_h, std::vector< std::vector<int> > &result, int start, int end)
{
...
int main(int argc, char **argv)
{
	...
	transitive_closure_setup(g_d,start,end,result);
	if(op.verify)
	{
		verify(g_h,result,start,end) ? std::cout << "Test passed." : std::cout << "Test failed.";
		std::cout << std::endl;
	}
